<template>
  <div class="page">
    ...
    <div class="photo-wall">
      <el-image v-for="item in loadedPhotos" :key="item.id" class="photo-item" :src="item.thumbnail" fit="cover"
        :preview-src-list="[item.url]" loading="lazy" hide-on-click-modal />

      <!-- 加载状态提示 -->
      <div v-if="loading" class="loading-tip">正在加载中...</div>
      <div v-if="noMore" class="no-more-tip">已经到底了</div>
    </div>
    <!-- 滚动检测哨兵 -->
...
const initPhotosData = () => {
  ...
  for (let i = 0; i < MAXNUMBER; i++) {
    photoList.push({
      id: i + 1,
      thumbnail: `${ossUrl}${photosFolder}${i + 1
        }.jpg?x-oss-process=image/resize,w_${ITEM_HEIGHT},h_${ITEM_HEIGHT},m_fill`,
