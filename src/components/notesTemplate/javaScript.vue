...
export default {
  setup() {
    const list = reactive([
      {
        id: 1, title: '关于ES6中 var/let/const 的个人理解', name: 'a1', html: `先说区别 </br>
         ...
         ps. 关于第四点的补充 默认情况使用const创建变量，当且仅当已知某值会被修改时才使用let创建`
      },
      {
        id:2,title:'关于拓展运算符...刚学到的一些应用',name:'a2',html:`假如给定一个数组 arr = [11,22,14,12,33,21,35] 需要获取到最大值，我首先的想法是对数组排序(sort)然后拿最后一位（length-1）</br>
        在其他地方看到，完全可以使用Math来进行纯数学运算，只需要通过Math.max(...arr)即可`
      },
      {
        id:3,title:'关于 箭头函数 的个人理解',name:'a3',html:`在没看书之前，我自己使用箭头函数最多的是用在保持this指针指向(vue中),原来es6中箭头函数有这么多特性特点，仅做mark一下。</br>
        ...
        3 箭头函数是没有arguments对象 接受参数时需要通过形式参数或者剩余参数(...)的方式获取函数的参数 （如果在函数体内使用，获取到的是上一级的arguments）`
      },
      {
        id:4,title:'关于 ... 拓展符最近踩的坑',name:'a4',html:`最近项目中遇到一个相对复杂的表单信息，页面大概5个左右，每个页面之间需要互相通过$route.query传递7-8个参数信息，
        前人通过 ...query 的方式将数据向下传递，在某页中，其中一个key需要改变，但是在下一页发现该数据总是拿不到最新的值。排查问题发现，页面中某处使用了 两次拓展符对query进行了拓展，
        出现的问题就是后面的拓展的对象中的key值覆盖了前面需要的key值。</br>所以在使用拓展符时首先要确认的是原对象内的key值，以及目标对象内的key值，如果存在冲突，将需要保存的内容还在对象尾部，进行覆盖。`
      },
      {
        id:5,title:'关于业务前置和业务后置的问题',name:'a5',html:`开发时时常会遇到前后端都能实现的功能到底谁来实现的问题，有人说需要看业务改动情况，有人说看影响范围，有人说具体情况具体分析。
        ...
        逻辑严谨程度打折扣（数据以前端为准），用户体验打折扣（app体量增大），源数据来源于数据库，处理在前端，项目的使用分配头重脚轻，核心业务由前端的处理数据不安全，不准确。`
      },
      {
        id:6,title:'关于前端项目优化的一个思路',name:'a6',html:`最近做了一个关于图片展示优化的活，从用户体验的角度来看，确实比之前“流畅”了一些，
        实现的方案是这样的，1取消promise.all事件，改为单个图片加载.2图片新增加载失败的函数处理（重试）.3关闭全局loading改为每个小图片的loading，不阻塞用户操作。
        以上改动点属于纯前端操作，接口还是原本的响应速度，但是从用户体验上来讲，用户觉得页面“响应”要比之前快多了，而且没有全局的loading阻塞用户行为，体验感提升了不少。`
      },
      {
        id:7,title:'JS的计时器是否准确？为什么？',name:'a7',html:`分为四个层面。1硬件。不存在绝对意义的精确计时，相对意义精确的计时是原子钟，计算机硬件并非原子钟，所以硬件方面计时并不准确。2软件。浏览器中js代码
        ...
        间隔是4ms(毫秒)。4事件循环。计时器到达后也不一定会马上执行，而是要等待执行栈清空后才会执行。`
      },
    ])
    let activeName =  ref("");
